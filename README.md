...
> **送信は、AIにさせない。── コードで検証できる人間の承認なしには。**
> 食品D2Cブランドのカスタマーサポートを、*人間承認フロー付き*で自動化するRAGエンジン。
> 事実はすべてナレッジベースから。嘘をつかせない仕組みを、プロンプトではなく **コードで** 縛る。
> （初期は「送信のコードパスが物理的に存在しない」設計。現在は「**事前登録された承認者本人のリアクションでしか送信コードに到達できない**」設計へ進化 ── 詳細は[設計思想](#設計思想--送信だけはaiにさせない)）
...
問い合わせメール（あるいは LINE ）を受け取り、**ナレッジベースに根拠のある返信文の下書き** を生成するエンジンです。
下書きは社内のDiscord承認チャンネルに投稿され、**送信されるのは、決裁者本人の👍（コードで検証される承認）があったときだけ** です。
承認者の自由記述の返信は「修正指示」として解釈され、AIが新しい下書きを作り直します。
...
|---|---|---|
| AIが勝手に顧客へ送信しない | 「送信しないで」と指示 | 送信に到達する唯一のコードパスは、**事前登録された承認者本人のDiscordユーザーIDからの👍リアクション** でしかトリガーされない（初期は `sendMessage` のコードパス自体が不存在。後述の進化を参照） |
| 承認された送信だけ許可 | 「承認を待って」と指示 | 他人の👍・Bot自身のリアクション・返信のみ・無反応では、`sendDraft()` に **一切到達しない**（コードの分岐条件で判定） |
...
安全契約（承認なし送信の禁止・エスカレ・署名）は再検証なしで保たれる。** これがこのプロダクトの背骨です。

### 「送信しない」の進化（2026-07）

初期の実装は「エンジンに送信のコードパスがそもそも存在しない」でした。運用が回り始めてから、
承認の場を Gmail の下書きフォルダから **Discord Bot** に移し、安全モデルを一段抽象化しました：

> 「AIは送信しない」から「AIは、**コードで検証可能な人間承認** を経由しない限り送信しない」へ。

送信できる唯一のコードパスは Discord フロー1箇所で、**事前登録された決裁者本人のDiscordユーザーIDからの
👍リアクション** だけをトリガー条件にしています。これは縛りが弱くなったのではなく、
「プロンプトではなくコードで縛る」という設計哲学をそのまま保ったうえで、
承認〜送信の往復をスマホの👍一つに圧縮した、**設計思想の成熟** です。
詳細は [`docs/safety-rules.md`](docs/safety-rules.md)。
...
{ predictedMode, effectiveMode }   ← 既定は必ず human_approval（＝人間承認）
  ...
  ▼
Gmail 下書き ＋ Discord承認チャンネルへ投稿
  │  承認者本人の👍 → sendDraft() ／ 返信（自由記述）→ reviseLLM() が新しい下書きを再生成
  ▼
顧客へ送信（承認された下書きのみ）
```
...
└─────────┘           │        └────────────────┘                   └──────────────┘
    │                 └──────────────────┬──────────────────────────┘
    │ 承認者の👍で                        │ 全下書きを投稿
    │ sendDraft() → 顧客へ送信           │ （同じ5分Cronでリアクション/返信をポーリング）
    │                                    ▼
    │                 ┌──────────────────────────────────┐
    └─────────────────│ Discord 承認チャンネル（社内）      │
                      │  👍（承認者本人のIDと厳密一致）→送信 │
...
- **送信のトリガーは、この図の下の人間にしか存在しない。** エンジンが `sendDraft()` を呼ぶのは、
  Discord上で決裁者本人の👍リアクションをコードで検証できたときだけです。
- Discordの受信も常時接続（Gateway）ではなく、**同じ5分Cronに相乗りしたポーリング** です。シンプル志向は変えていません。
...
### 1. AIは検証済み人間承認を経由しない限り送信しない

送信に到達する唯一のコードパスは、**事前登録された決裁者本人のDiscordユーザーID** からの
👍リアクションでしかトリガーされません。他人の👍・Botの反応・返信のみ・無反応では `sendDraft()` に一切到達しない ──
「送りたい」と思ったモデルがいても、承認者の👍という **コードの分岐条件** を偽造できないので送れません。

さらに二段構えの安全チェックがあります：

- **必須エスカレ7カテゴリは例外なく個別承認**（`escalate=true` は自動送信経路の入口で常に除外）
- **カテゴリ単位の自動送信解禁**（「もう10回連続ノーエディット承認」等の実測をトリガーに、Discordの2択質問で決裁者が👍/👎で確定。AIが自分で解禁することはできない）が通っていても、
  個々のメールが未確定スロット・KB鮮度警告・低確信度のいずれかに該当すれば、その1通だけは人間承認に戻る

（初期実装は「`sendMessage` のコードパス自体が存在しない」でした。この進化の経緯は[設計思想](#送信しないの進化2026-07)参照）
...
- **何を作るか／何を絶対にさせないか**: 承認フローを残す、承認なしには送信させない、というプロダクトの芯
- **安全設計の発想**: 「プロンプトではなくコードで縛る」というアーキテクチャ上の意思決定（初期の「`sendMessage` を消す」も、現行の「承認者IDの厳密一致だけを送信トリガーにする」も、この同じ発想の実装）
- **Discordピボットの意思決定**: 承認チャンネルの一本化、顧客メールをどこまで表示するか（社内非公開コンソールだから全文表示、というPII方針）、カテゴリ自動送信を解禁する閾値と「解禁の最終決定は必ず人間の👍」という原則
- **レビュー基準と、どこまで自動修正させるかの判断**: AIに実装させたコードを、どの角度でレビューし、見つかった欠陥のどれを直させるか（後述）
...
**「作って終わり」にしないレビュー工程（Discordピボット時の実例）:**

Discord承認ゲートの実装後、本番反映の前に **10エージェント並列の敵対的コードレビュー**
（正しさ5角度＋クリーンアップ3角度＋設計妥当性＋規約準拠、検証投票つき）を自分でかけました。
結果、**実際に15件の欠陥** を検出して全部修正してから出しています。安全に関わる代表例：

- 新設のカテゴリ自動送信が、既存の送信ゲートの安全3条件（未確定スロット・KB鮮度・確信度）を見ずに
  「カテゴリが解禁済みか」だけで判定していた ── **新機能を足すとき、既存の安全機構を無意識に迂回してしまう**
  というAI実装の典型パターンの実例
- 下書き生成に失敗する異常系で、必須エスカレの通知が発火せず最重要案件が誰にも知られないまま「処理済み」になるパス
- LLM再生成が失敗しても「反映しました」と表示してしまい、変わっていない下書きを誤承認させうるバグ
- 承認者IDの環境変数が1つ抜けているだけで、承認機構全体がエラーも出さず静かに停止する構造（起動時必須化で修正）

「バグゼロで書けた」ではなく、**AIに実装させたら敵対的にレビューさせてから出す** ── この工程自体を成果物と考えています。
...
**③ Cloudflareのcronトリガー上限に、本番デプロイで初めてぶつかった。**
Discord承認フローを作ったとき、リアクションを拾う専用cronを1本足す設計にしていました。ところが
Cloudflareには **アカウント全体でcronトリガー5個** という上限があり、本番デプロイ時に初めて発覚。
「既存の5分tickに相乗りさせる」設計に変更しました。プラットフォームの制約は、本番に当ててみるまで分からない。

**④ 「手動コマンドで再生成」設計を、一度実装まで進めてから作り直した。**
下書きの修正フローは、最初「自分がClaude Codeセッションを開いて再生成コマンドを叩く」設計で実装まで進めました。
しかし **「自分が毎回セッションを開く」こと自体がボトルネック** だと気づき、`git checkout` で差分を破棄して
常時稼働のDiscord Bot（承認者がDiscordで返信するだけ）へ仕切り直しました。
最初の設計に固執せず、要件が見えてから作り直す ── この判断も含めて一次情報として残します。
...
- ~~**エスカレーションの導線。**~~ → **Discordピボットで実質解決。** 全下書き・エスカレ通知がDiscordの承認チャンネルに
  集約され、承認・修正指示・解禁までそこで完結するようになりました（会話ログがそのまま監査ログになる）。
- **Discordポーリングの5分遅延。** 承認の👍を拾うのが5分Cron相乗りのポーリングなので、👍から実送信まで最大5分のラグがある。
  Durable Object 化などで縮める余地。
- **自動送信解禁の可視化・取り消しUI。** カテゴリ解禁は決裁者の👍で確定するが、「今どのカテゴリが解禁済みか」を
  一覧で見たり、ワンタップで取り消したりするUIはまだ無い。
